...
function html(message,ok=true){return `<!doctype html><html><body style="font-family:system-ui;background:#09080d;color:#eee;display:grid;place-items:center;height:100vh"><div style="max-width:520px;padding:32px;text-align:center"><h2>${ok?"🦉 HudHud account connected":"Connection error"}</h2><p>${message}</p><p>You can close this window.</p></div><script>setTimeout(()=>window.location.href="/?view=connections",1200)</script></body></html>`;}
...
export default async function handler(req,res){
 ...
 if(req.method==="GET"&&action==="start"){
   ...
   const state=crypto.randomBytes(24).toString("hex");let statePayload=state;
