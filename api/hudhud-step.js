...
function baseUrl(){return String(env("HUDHUD_SUPABASE_URL")||"").trim().replace(/\/+$/,"").replace(/\/(?:rest\/v1|auth\/v1)$/i,"");}
async function verifyUser(req){
  const base=baseUrl(),key=env("HUDHUD_SUPABASE_KEY");
...
async function callBrain(message){
  const brainUrl=env("HUDHUD_BRAIN_URL");
  if(!brainUrl)throw new Error("HudHud brain is not configured.");
  const response=await fetch(brainUrl,{method:"POST",headers:{"Content-Type":"application/json",...(env("HUDHUD_BRAIN_TOKEN")?{Authorization:`Bearer ${env("HUDHUD_BRAIN_TOKEN")}`}: {})},body:JSON.stringify({message})});
  const raw=await response.text();let data;try{data=JSON.parse(raw)}catch{data={reply:raw}};
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(detected){
      try{toolRun={tool:detected.tool,args:detected.args,result:await routeTool(detected.tool,detected.args)};}catch(error){toolRun={tool:detected.tool,error:error.message};}
    }
