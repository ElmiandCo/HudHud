...
const PROVIDERS = {
  ...
  tiktok: {
    ...
    scopes: String(process.env.TIKTOK_SCOPES || "user.info.basic,user.info.profile,user.info.stats,video.list").split(",").map(x => x.trim()).filter(Boolean),
...
export function buildAuthorizeUrl(req, provider, userId) {
  ...
  const params = new URLSearchParams({
    response_type: "code",
    [provider === "tiktok" ? "client_key" : "client_id"]: clientId,
...
export async function fetchProfile(provider, accessToken) {
  ...
  if (provider === "tiktok") {
    const data = await bearerGet("https://open.tiktokapis.com/v2/user/info/?fields=open_id,union_id,avatar_url,display_name,username,bio_description,profile_deep_link,is_verified,follower_count,following_count,likes_count,video_count", accessToken);
  ...
  if (provider === "tiktok") {
    return {
      ...
      stats: {
        ...
        videos: Number(profile.video_count || 0),
      },
    };
  }
  if (provider === "x") {
...
export async function saveOAuthConnection(userId, provider, token, profile, req) {
  ...
  if (!url || !key) throw new Error("Supabase server credentials are not configured. Set HUDHUD_SUPABASE_URL and HUDHUD_SUPABASE_SERVICE_ROLE_KEY (or SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY).");
  ...
  if (!existingResponse.ok) {
    ...
    throw new Error("OAuth token lookup failed (" + existingResponse.status + "): " + String(detail).slice(0, 300));
  }
  ...
  if (!tokenResponse.ok) {
    ...
    throw new Error("OAuth token storage failed (" + tokenResponse.status + "): " + String(detail).slice(0, 300));
  }
  ...
  if (!integrationResponse.ok) {
    const detail = await integrationResponse.text().catch(() => "");
    throw new Error("Integration lookup failed (" + integrationResponse.status + "): " + String(detail).slice(0, 300));
  }
  ...
  const integrationRow = {
    ...
    metadata: { profile_url: normalized.profileUrl, avatar_url: normalized.avatarUrl, bio: normalized.bio || null, is_verified: normalized.isVerified === true, stats: normalized.stats || null },
