...
function connections(){return '<div class="section-head"><div><span class="eyebrow">CONTROL / SETTINGS</span><h2>Connections</h2><span class="muted">Live connection state for the systems HudHud can use.</span></div><button class="primary" data-connection-refresh>↻ Check all connections</button></div><div class="connection-control-grid"><div class="card connection-control-card"><span class="status-light status-unknown" data-conn-light="github"></span><div class="connection-logo">🐙</div><h3>GitHub</h3><p>Repositories, files, issues and commits.</p><div class="connection-status-line"><strong data-conn-status="github">Checking…</strong><span data-conn-detail="github">—</span></div></div><div class="card connection-control-card"><span class="status-light status-unknown" data-conn-light="vercel"></span><div class="connection-logo">▲</div><h3>Vercel</h3><p>Projects, deployments and runtime operations.</p><div class="connection-status-line"><strong data-conn-status="vercel">Checking…</strong><span data-conn-detail="vercel">—</span></div></div><div class="card connection-control-card"><span class="status-light status-unknown" data-conn-light="supabase"></span><div class="connection-logo">⚡</div><h3>Supabase</h3><p>Tables, data and backend services.</p><div class="connection-status-line"><strong data-conn-status="supabase">Checking…</strong><span data-conn-detail="supabase">—</span></div></div></div><div class="card connection-settings"><div><div class="muted">SITE CONTROL</div><h3>Connection state is managed here.</h3><p>Use <b>Check all connections</b> to refresh live state. Secrets are never entered or displayed in this browser; they stay server-side in Vercel.</p></div><div class="settings-badge"><span class="status-light status-good"></span> Server-side credentials</div></div><div class="card"><div class="muted">RECORDED CONNECTIONS</div><h3>Workspace notes</h3>'+list(state.connections,"No manual connections recorded.","Add a human-readable connection note if you want it tracked in your workspace.")+'</div>';}
...
function updateConnectionRows(conns){Object.keys(conns).forEach(name=>{const d=conns[name]||{},status=d.status||"unknown";document.querySelectorAll("[data-live-connection=\""+name+"\"]").forEach(row=>{const light=row.querySelector(".status-light"),em=row.querySelector("em");if(light)light.className="status-light "+(status==="connected"?"status-good":status==="auth_error"?"status-warn":"status-bad");if(em)em.textContent=d.detail||status.replace("_"," ");});document.querySelectorAll("[data-conn-status=\""+name+"\"]").forEach(el=>el.textContent=status==="connected"?"CONNECTED":status==="auth_error"?"AUTH ERROR":status==="not_configured"?"NOT CONFIGURED":"OFFLINE");document.querySelectorAll("[data-conn-detail=\""+name+"\"]").forEach(el=>el.textContent=d.detail||"—");document.querySelectorAll("[data-conn-light=\""+name+"\"]").forEach(el=>el.className="status-light "+(status==="connected"?"status-good":status==="auth_error"?"status-warn":"status-bad"));});}
...
function bindConnections(){document.querySelectorAll("[data-connection-refresh]").forEach(b=>b.onclick=refreshConnections);refreshConnections();}
