...
function commandCenter(){
 const defs=[["github","🐙","GitHub","Repositories, files, issues and pull requests","Connected"],["vercel","▲","Vercel","Projects, deployments and production","Connected"],["supabase","⚡","Supabase","Database, authentication and resources","Connected"],["stripe","💳","Stripe","Products, prices and checkout","Connected"],["hudhud","🦉","HudHud","Command layer, workspace and tools","Online"],["brain","🧠","HudHud Brain","Reasoning and local AI bridge","Online"],["openclaw","🦞","OpenClaw","Local computer-side actions","Local"]], placeholders=[["☁️","AWS","Cloud infrastructure"],["☁️","Azure","Cloud infrastructure"],["☁️","Google Cloud","Cloud infrastructure"],["✉️","Gmail","Email and communication"],["📅","Google Calendar","Scheduling and events"],["💬","Slack","Team communication"],["📋","Notion","Knowledge and docs"],["🟣","Linear","Project management"],["🟠","Jira","Engineering workflows"],["💼","Microsoft 365","Business productivity"],["🤖","OpenAI","AI provider"],["🤖","Anthropic","AI provider"],["✨","Google Gemini","AI provider"],["🤖","Groq","AI provider"],["🤖","xAI","AI provider"]];
 return '<div class="command-center"><div class="command-hero"><div><span class="eyebrow">HUDHUD COMMAND CENTER</span><h2>Ecosystem Control & Diagnostics</h2><p>Monitor every connected service, inspect resources, and diagnose problems from one operational view.</p></div><div class="command-hero-actions"><button class="primary" data-command-diagnostic>🔎 Run Full Diagnostic</button><button class="secondary" data-command-add-tool>＋ Add Tool</button></div></div><div class="command-summary"><div><strong id="commandHealthCount">—</strong><span> services operational</span></div><div id="commandLastCheck">Not checked yet</div></div><section class="command-ecosystem card"><div class="command-section-head"><div><span class="eyebrow">LIVE ECOSYSTEM</span><h3>Connected infrastructure</h3></div><span id="commandDiagnosticState" class="system-state">READY</span></div><div class="ecosystem-map"><div class="ecosystem-core"><div class="ecosystem-orb">🦉</div><strong>HUDHUD</strong><small>COMMAND LAYER</small></div><div class="ecosystem-links">'+defs.slice(0,4).map(x=>'<button class="ecosystem-node" data-command-node="'+x[0]+'"><span>'+x[1]+'</span><strong>'+x[2]+'</strong><small>● '+x[4]+'</small></button>').join('')+'</div></div></section><section class="command-grid"><div class="card command-health-card"><div class="command-section-head"><div><span class="eyebrow">SYSTEM HEALTH</span><h3>Live connection status</h3></div></div><div id="commandHealthList">'+defs.map(x=>'<div class="command-service" data-command-service="'+x[0]+'"><span class="command-service-icon">'+x[1]+'</span><div><strong>'+x[2]+'</strong><small>'+x[3]+'</small></div><span class="command-status status-unknown">CHECKING</span></div>').join('')+'</div></div><div class="card command-diagnostic-card"><div class="command-section-head"><div><span class="eyebrow">DIAGNOSTICS</span><h3>Diagnostic report</h3></div></div><div id="commandDiagnosticLog" class="command-diagnostic-log"><div class="command-log-empty">Run a full diagnostic to inspect authentication, resource access and service health.</div></div></div></section><section class="card command-resources"><div class="command-section-head"><div><span class="eyebrow">RESOURCE MAP</span><h3>Connected resources</h3><p>Resources HudHud can discover through configured connections.</p></div></div><div id="commandResourceGrid" class="command-resource-grid"></div></section><section class="card command-add-tool"><div class="command-section-head"><div><span class="eyebrow">EXTEND HUDHUD</span><h3>＋ Add a tool</h3><p>Future integrations can plug into this ecosystem without changing the Command Center.</p></div></div><div class="command-tool-grid">'+placeholders.map(x=>'<div class="command-tool-placeholder"><span>'+x[0]+'</span><div><strong>'+x[1]+'</strong><small>'+x[2]+'</small></div><em>Coming soon</em></div>').join('')+'</div></section><div id="commandToolModal"></div></div>';
}
async function runCommandDiagnostic(){
 const stateEl=document.getElementById("commandDiagnosticState"),logEl=document.getElementById("commandDiagnosticLog"),listEl=document.getElementById("commandHealthList"),countEl=document.getElementById("commandHealthCount"),timeEl=document.getElementById("commandLastCheck");
 if(stateEl)stateEl.textContent="RUNNING…";if(logEl)logEl.innerHTML='<div class="command-log-item">🔎 Checking configured connections…</div>';
 const started=Date.now();
 try{const r=await fetch("/api/connection-status",{cache:"no-store"}),data=await r.json();if(!r.ok)throw new Error(data.error||"Connection diagnostic failed.");let operational=0,items=[];
 Object.entries(data.connections||{}).forEach(([name,d])=>{const ok=d.status==="connected";if(ok)operational++;items.push((ok?"🟢":"🔴")+" "+name+" — "+(d.message||d.status||"unknown"));const row=listEl?.querySelector('[data-command-service="'+name+'"]');if(row){const st=row.querySelector(".command-status");st.textContent=ok?"CONNECTED":"ERROR";st.className="command-status "+(ok?"status-good":"status-bad");}});
 [["hudhud",true,"Runtime available"],["brain",true,"Control layer available"],["openclaw",false,"Local service not reported by browser"]].forEach(([name,ok,msg])=>{if(ok)operational++;items.push((ok?"🟢":"🟡")+" "+name+" — "+msg);const row=listEl?.querySelector('[data-command-service="'+name+'"]');if(row){const st=row.querySelector(".command-status");st.textContent=ok?"ONLINE":"NOT REPORTED";st.className="command-status "+(ok?"status-good":"status-warn");}});
 const good=items.every(x=>x.startsWith("🟢"));if(countEl)countEl.textContent=operational;if(timeEl)timeEl.textContent="Last checked "+new Date().toLocaleTimeString();if(stateEl){stateEl.textContent=good?"ALL SYSTEMS OPERATIONAL":"ATTENTION NEEDED";stateEl.className="system-state "+(good?"good":"warn");}if(logEl)logEl.innerHTML=items.map(x=>'<div class="command-log-item">'+esc(x)+'</div>').join('')+'<div class="command-log-item muted">Completed in '+(Date.now()-started)+' ms.</div>';loadCommandResources();
 }catch(e){if(stateEl){stateEl.textContent="DIAGNOSTIC FAILED";stateEl.className="system-state warn";}if(logEl)logEl.innerHTML='<div class="command-log-item">🔴 '+esc(e.message||String(e))+'</div>';}
}
async function loadCommandResources(){
 const host=document.getElementById("commandResourceGrid");if(!host)return;const token=await authAccessToken();const providers=[["github","🐙","GitHub"],["vercel","▲","Vercel"],["supabase","⚡","Supabase"]];const rows=[];
 for(const [provider,icon,label] of providers){try{const r=await fetch("/api/connection-resources?provider="+provider,{headers:token?{Authorization:"Bearer "+token}:{}}),d=await r.json(),resources=d.resources||[];rows.push('<div class="command-resource-card"><span>'+icon+'</span><strong>'+label+'</strong><small>'+resources.length+' resource(s) discovered</small><div class="command-resource-list">'+resources.slice(0,6).map(x=>'<span>'+esc(x.name||x.full_name||x.id)+'</span>').join('')+'</div></div>')}catch(e){rows.push('<div class="command-resource-card"><span>'+icon+'</span><strong>'+label+'</strong><small>Unable to inspect resources</small></div>')}}host.innerHTML=rows.join('');
}
function openCommandToolModal(){const host=document.getElementById("commandToolModal");if(!host)return;const p=[["☁️","AWS","Cloud infrastructure"],["☁️","Azure","Cloud infrastructure"],["☁️","Google Cloud","Cloud infrastructure"],["✉️","Gmail","Email and communication"],["📅","Google Calendar","Scheduling and events"],["💬","Slack","Team communication"],["📋","Notion","Knowledge and docs"],["🟣","Linear","Project management"],["🟠","Jira","Engineering workflows"],["💼","Microsoft 365","Business productivity"],["🤖","OpenAI","AI provider"],["🤖","Anthropic","AI provider"],["✨","Google Gemini","AI provider"],["🤖","Groq","AI provider"],["🤖","xAI","AI provider"]];host.innerHTML='<div class="resource-modal"><div class="resource-backdrop" data-close-command-tool></div><div class="resource-dialog"><button class="resource-close" data-close-command-tool>×</button><div class="eyebrow">ADD TOOL</div><h2>Connect another service</h2><p class="resource-subtitle">Choose a provider. Authentication and permissions will be added as integrations are enabled.</p><div class="command-tool-grid command-tool-modal-grid">'+p.map(x=>'<button class="command-tool-placeholder"><span>'+x[0]+'</span><div><strong>'+x[1]+'</strong><small>'+x[2]+'</small></div><em>Coming soon</em></button>').join('')+'</div></div></div>';host.querySelectorAll("[data-close-command-tool]").forEach(b=>b.onclick=()=>host.innerHTML="");}
function showCommandNode(key){const names={github:"GitHub",vercel:"Vercel",supabase:"Supabase",stripe:"Stripe"};const host=document.getElementById("commandToolModal");if(host)host.innerHTML='<div class="resource-modal"><div class="resource-backdrop" data-close-command-tool></div><div class="resource-dialog"><button class="resource-close" data-close-command-tool>×</button><div class="eyebrow">CONNECTION</div><h2>'+names[key]+'</h2><p class="resource-subtitle">Use Connections to configure accounts and resource scope. Command Center diagnostics will report connection health here.</p><div class="form-actions"><button class="primary" data-open-connections>Open Connections</button></div></div></div>';document.querySelector("[data-open-connections]")?.addEventListener("click",()=>{host.innerHTML="";render("connections")});}
function bindCommandCenter(){document.querySelectorAll("[data-command-diagnostic]").forEach(b=>b.onclick=runCommandDiagnostic);document.querySelectorAll("[data-command-add-tool]").forEach(b=>b.onclick=openCommandToolModal);document.querySelectorAll("[data-command-node]").forEach(b=>b.onclick=()=>showCommandNode(b.dataset.commandNode));}
