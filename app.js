...
async function startSocialOAuth(provider){
  ...
  try{
    ...
    const message=e.message||"Social authorization failed";
    console.error("HudHud OAuth start failed",e);
    const panel=document.getElementById("socialOAuthError");
    if(panel){panel.hidden=false;panel.innerHTML="<strong>OAuth could not start</strong><span>"+esc(message)+"</span><small>Check the browser console only if needed; do not paste client secrets here.</small>";}
    toast(message);
...
function bindGameLab(){
 const create=document.getElementById("gameCreate"),download=document.getElementById("gameDownload"),status=document.getElementById("gameStatus"),plan=document.getElementById("gamePlan"),title=document.getElementById("gamePlanTitle");let blueprint=null;
 const build=(concept,engine,target,aiAssets,multiplayer,monetization)=>{
  const en={unity:"Unity",cocos:"Cocos Creator",laya:"LayaAir"}[engine],tn={tiktok:"TikTok Mini Games",web:"Web / PWA",mobile:"Mobile app"}[target];
  const steps=[["Game design","Turn the concept into the core loop, controls, win/lose states, progression, and asset list."],["Engine workspace","Create the source project in "+en+" and keep source code/assets in the engine project."],["AI production",aiAssets?"Use AI to accelerate scripts, prototypes, sprites, textures, sounds, animations, and materials.":"Use approved human-created assets."]];
  const req=[];
  if(target==="tiktok"){req.push(["TikTok developer organization","Create/verify the business organization and Mini Game app."],["TikTok qualification","Complete business/industry qualification and regional launch approval where required."],["TikTok SDK capabilities","Integrate required capabilities such as silent login, rewarded ads, home-screen shortcut, and revisit-from-profile."],["TikTok DevTool","Export a TikTok-compatible native package, debug it, and test on a real device."],["Release workflow","Upload, preview, test, submit for review, and release the validated package."]);}else if(target==="web"){req.push(["Web hosting","Choose the deployment target and connect a domain if needed."],["Browser QA","Test mobile/desktop input, audio, loading, performance, and accessibility."],["Analytics","Connect an approved analytics provider if required."]);}else{req.push(["Mobile developer account","Create the Apple/Google developer account required for distribution."],["Native build","Configure signing, identifiers, icons, permissions, and release builds."],["Store submission","Complete testing, metadata, privacy declarations, and review."]);}
  if(multiplayer)req.push(["Multiplayer backend","Provision authoritative sessions, matchmaking, persistence, and abuse controls."]);if(monetization)req.push(["Monetization","Connect the target platform's approved ads/IAP or payment system; keep secrets server-side."]);
  return {concept,engine,engineLabel:en,target,targetLabel:tn,aiAssets,multiplayer,monetization,createdAt:new Date().toISOString(),steps,requirements:req};
 };
 create?.addEventListener("click",()=>{const concept=document.getElementById("gameConcept")?.value.trim();if(!concept){status.textContent="Describe the game first.";return;}blueprint=build(concept,document.getElementById("gameEngine").value,document.getElementById("gameTarget").value,document.getElementById("gameAiAssets").checked,document.getElementById("gameMultiplayer").checked,document.getElementById("gameMonetization").checked);const saved=JSON.parse(localStorage.getItem("hudhud_game_projects_v1")||"[]");saved.unshift(blueprint);localStorage.setItem("hudhud_game_projects_v1",JSON.stringify(saved.slice(0,20)));title.textContent=blueprint.targetLabel+" build plan";const items=[...blueprint.steps,...blueprint.requirements].map(x=>x[1]);plan.innerHTML='<div class="game-plan-list">'+items.map((x,i)=>'<div class="game-plan-row"><span>'+String(i+1).padStart(2,"0")+'</span><p>'+esc(x)+'</p></div>').join("")+'</div><div class="game-platform-note"><strong>HudHud will request what is actually needed.</strong><span>Accounts, memberships, SDKs, credentials, and approvals are separate setup steps. Secrets belong server-side.</span></div>';download.disabled=false;status.textContent="Blueprint created and saved on this device. Ready for engine handoff.";log("Created Game Lab blueprint: "+blueprint.concept);});
 download?.addEventListener("click",()=>{if(!blueprint)return;const blob=new Blob([JSON.stringify(blueprint,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="HudHud-Game-Blueprint.json";a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);status.textContent="Blueprint exported.";});
}
function bindStudio(){
